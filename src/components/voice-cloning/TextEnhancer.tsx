  ...
  return <div>
      <Tabs value={activeTab} onValueChange={onTabChange as (value: string) => void} className="w-full">
        ...
        <TabsContent value="input" className="space-y-4">
          <Textarea placeholder="Enter what you'd like this voice to say..." className="min-h-[120px] shadow-inner bg-voicevault-softgray/20 border-voicevault-softpurple/50 focus:border-voicevault-primary" value={inputText} onChange={onTextChange} />
          
          <div className="flex flex-wrap gap-2 mb-4">
            <span className="text-xs text-gray-500 mt-1 mr-1">Examples:</span>
            {examples.map((example, index) => <Badge key={index} variant="outline" className="cursor-pointer bg-voicevault-softgray/30 hover:bg-voicevault-softgray/50 text-voicevault-tertiary" onClick={() => handleExampleClick(example.text)}>
                {example.icon}
          ...
          <div className="flex justify-end">
            <Button onClick={onEnhance} disabled={!inputText.trim() || isEnhancing} className="bg-voicevault-softpeach text-voicevault-tertiary hover:bg-voicevault-softpeach/80">
              {isEnhancing ? <>
        ...
        <TabsContent value="enhanced" className="space-y-4">
          <Textarea className="min-h-[120px] shadow-inner bg-voicevault-softgray/10 border-voicevault-primary/30 text-voicevault-tertiary" value={enhancedText} readOnly />
          
          <div className="flex justify-between">
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => onTabChange("input")} className="text-voicevault-tertiary">
                Edit Original
              </Button>
              <Button variant="outline" onClick={onEnhance} disabled={isEnhancing} className="text-voicevault-tertiary">
